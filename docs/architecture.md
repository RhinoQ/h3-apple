...
日常 API 只服务 Ours；与 vpipe 的比较是独立可选脚本。
...
| `benchmarks/compare.py` | 直接调用 Ours / vpipe CLI，统一输入、外层计时和最终媒体检查 |
...
进程负责生命周期与发布。外部 vpipe 进程则由比较脚本持锁，避免重复获取同一把锁。
...
无需通用后端接口、插件注册表、常驻 HTTP 服务或额外共享核心仓库。当前 5 秒和
15 秒主表均为 Ours / vpipe VDN；日常研究配对产品候选与稳定版本。官方 FastH3
对照已取消，其历史结果和复核入口保留。
