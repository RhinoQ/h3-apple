...
from h3_apple._vendor.fastvideo_mlx.frame_upsample import (
    DEFAULT_PIXEL_UPSAMPLE_MODE,
    PIXEL_UPSAMPLE_MODES,
    upsample_frames,
)
from h3_apple._vendor.fastvideo_mlx.minimax_h3 import (
...
def plan_fast_spatial(
    ...
    *,
    ...
    :mod:`h3_apple._vendor.fastvideo_mlx.frame_upsample` for why.
    """
    if scale < 2:
...
def _preflight_media_dependencies(*,
    ...
    from h3_apple._vendor.fastvideo_mlx.rife_interp import ensure_weights_available
...
class MiniMaxH3MLXPipeline:
    ...
    def resolve_geometry(
        ...
        *,
        ...
        # The public API validates 5–15 s delivery and trims the 362-frame tail.
        if enforce_duration and not 5.0 <= duration <= 362 / MINIMAX_H3_FPS:
    ...
    def _load_conditioner(self):
        from h3_apple._vendor.fastvideo_mlx.minimax_h3_conditioner import StreamedMiniMaxH3TextConditioner
    ...
    def denoise(
        ...
        *,
        ...
        if self.observer is not None:
            dit.on_block = self.observer.block
        ...
        dit_forward_s = 0.0
        if self.observer is not None:
            self.observer.capture("initial-inputs", lambda: {
                "video": np.asarray(x_v), "audio": np.asarray(x_a), "text": np.asarray(text),
            })
        ...
        for step_index in range(num_steps):
            ...
            if self.observer is not None:
                self.observer.step(step_index, x_v, x_a, video_velocity, audio_velocity)
    ...
    def decode_video(self,
        ...
        if self.video_decode_backend == "taeh3":
            from h3_apple._vendor.fastvideo_mlx.minimax_h3_taeh3 import decode_latents_taeh3_mlx
        ...
        if self.observer is not None:
            self.observer.capture("video-preclamp", lambda: {"pixels": np.asarray(preclamp)})
    ...
    def _load_video_vae(self):
        from h3_apple._vendor.fastvideo_mlx.minimax_h3_video_vae import mlx_h3_video_vae_from_dir
        return mlx_h3_video_vae_from_dir(self.model_root / "vae", include_encoder=False,
    ...
    def decode_audio(self, audio_rows: np.ndarray, *, num_frames: int) -> np.ndarray:
        ...
        from h3_apple._vendor.fastvideo_mlx.minimax_h3_audio_vae import mlx_h3_audio_vae_from_dir
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "-b:a",
                    "192k",
                    # Both arrays have their delivery lengths; -shortest can
                    # discard the final video/AAC packet at fractional endpoints.
                    "-movie_timescale",
    ...
    def generate(
        ...
        if self.video_decode_backend == "taeh3":
            from h3_apple._vendor.fastvideo_mlx.minimax_h3_taeh3 import ensure_taeh3_checkpoint
        ...
        if fast_plan is not None:
            from h3_apple._vendor.fastvideo_mlx.rife_interp import interpolate_to_frame_count, load_model
