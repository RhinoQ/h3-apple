...
"""Experimental mixed-reference encoding on the H3 Apple execution chain.
...
from ..media import probe_reference, tool
...
from .ref2va import (ReferenceGeometry, build_ref2va_layout, build_ref2va_timesteps,
                    generated_rows, prepare_reference_image, reference_image_size)
...
def prepare_references(options, model_frames):
    ...
    for path in options.get("video_paths", []):
        ...
        height, width = reference_image_size(video["width"], video["height"], options["pixel_budget"])
        result = subprocess.run([tool("ffmpeg"), "-v", "error", "-i", str(path), "-t", str(duration),
...
def condition_and_denoise(request, options, checkpoint, observer, phase):
    if request["num_steps"] != 4 or options.get("attention") != "vsa":
        raise ValueError("This experimental mixed-reference path requires four-step Ref2VA VSA.")
    recipe = json.loads((Path(checkpoint) / "ref2va_recipe.json").read_text())
    if recipe.get("task") != "ref2va" or recipe.get("gate_tensors") != 50 or recipe.get("fasth3_t2va_deltas_applied") is not False:
    ...
    encoded = phase("conditioning", lambda: encode_references(Path(options["native_root"]),
    ...
    metadata = dict(task="ref2va", attention="vsa", experimental=True, reference_segments=ref_metadata,
    ...
    def denoise():
        ...
        dit.configure_vsa(MiniMaxH3VSAConfig(enabled=True, sparsity=.75, prefix_mode="exempt", impl="simd", routing_mode="kablex"))
        before = sparse_calls()
        result = sample_mixed(dit, text, fixed_video, fixed_audio, layout, video, audio, observer)
        stats = asdict(dit.last_vsa_stats)
        if (observer.nfe, observer.blocks, sparse_calls() - before) != (4, 200, 200) or stats["fallback_reasons"]:
