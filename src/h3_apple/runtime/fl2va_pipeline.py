...
"""First/last-frame conditioning with pinned LightX2V v1.2 weights.
...
from .ref2va_conditioning import encode_images
...
def condition_and_denoise(request, options, checkpoint, observer, phase):
    ...
    sampling = fl2va_sampling(recipe, request["num_steps"])
    references = []
    ...
    text, tags, rows, encoded = phase("conditioning", lambda: encode_images(
        Path(options["native_root"]), references, request["prompt"], observer))
    ...
    metadata = dict(task="fl2va", attention=attention, sampling=sampling, anchors=options["anchors"],
        encoded=encoded, condition_rows=len(condition), condition_sha256=checksum(condition),
        initial_video_sha256=checksum(initial_video), initial_audio_sha256=checksum(initial_audio),
        ...
        vsa_gates_used=attention == "vsa", video_shift=sampling["video_shift"], audio_shift=3)
    if attention == "vsa":
    ...
    def denoise():
        ...
        before = sparse_calls()
        video, audio = sample_image(dit, text, condition, layout, initial_video, initial_audio,
                                    observer=observer, video_shift=sampling["video_shift"])
