...
def test_keyframes_resolve_to_a_dedicated_model_task(tmp_path):
    ...
    request = resolve("gentle motion", first_frame=image, last_frame=image, aspect_ratio="9:16", seed=42)
    assert request.task == "fl2va" and request.resolution == "576p"
    assert request.first_frame == request.last_frame == str(image)
    assert (request.model_width, request.model_height) == (576, 1024)
