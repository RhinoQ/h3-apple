...
# Graph interface follows tgo-app-dev/vpipe, Apache-2.0, pinned in vpipe_assets.py.
...
from .image_inputs import prepare_keyframe, prepare_reference_image
...
def build_graph(request, assets, prepared, output):
    ...
    if request["task"]=="ref2va":
        ...
        add("diffusion-conditioner", inputs=(("text-prompt",0),("",0),("model-select",0)), unload_when_idle="always")
        ports[0]=("diffusion-conditioner",0)
    ...
    return dict(id="h3-apple-"+request["preset"],stages=stages,subpipelines=[])


def prepare_inputs(request, references, directory):
    if request["reference_videos"] or request["reference_audio"]:
        raise ValueError("Native reference video/audio has not been integrated; use the ours preset.")
    prepared=[]
    if references is None: return prepared
    for index,path in enumerate(references["image_paths"]):
        with Image.open(path) as image:
            if request["task"]=="fl2va":
                pixels=prepare_keyframe(image,request["model_width"],request["model_height"],stretch=index==0)
            else:
        ...
        if request["task"]=="fl2va": entry["anchor"]=references["anchors"][index]
...
def run(request, assets, workspace, emit, *, references=None, diagnostics=False):
    ...
    emit(dict(phase="native_generation",message="Running vpipe with "+request["preset"]))
        ...
        with (workspace/"native.log").open("x") as log:
            ...
            for line in process.stdout:
                ...
                if event is not None:
                    emit(event)
            if process.wait()!=0: raise RuntimeError(f"vpipe exited {process.returncode}; see native.log in the preserved workspace.")
    finally:
    ...
    log=(workspace/"native.log").read_text(); audit_log(log,request["preset"])
    ...
    return dict(actual_nfe=4,vsa=None,sparse_implementation="vpipe-sol-sage",
        ...
        native=dict(graph=graph,graph_sha256=digest(graph_path),log_sha256=digest(workspace/"native.log"),
            ...
            rng="Native vpipe RNG; equal integer seeds do not imply equal MLX initial noise.",delivery_command=delivery))
