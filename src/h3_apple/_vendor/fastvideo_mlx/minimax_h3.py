...
from h3_apple._vendor.fastvideo_mlx.fastwan import (
    MLXQuantizationSpec,
    QuantizedMatrix,
...
def _feed_forward(weights: dict[str, Any], x):
    ...
    from h3_apple.runtime.swiglu import bounded_swiglu

    hidden = linear(x, weights["ff.net.0.proj.weight"])
    return linear(bounded_swiglu(hidden), weights["ff.net.2.weight"])
    ...
    def forward_with_cache(
        ...
        *,
        ...
        for block_index, (block, tables) in enumerate(zip(self.blocks, cache.block_tables, strict=True)):
            ...
            mx.eval(packed)
            if getattr(self, "on_block", None) is not None:
                self.on_block(step_index, block_index + 1, len(self.blocks))
