...
def resolve_impl(requested: VSAImpl, head_dim: int, tile_elems: int) -> str:
    ...
    from h3_apple.runtime.sparse import simd_kernel_available, simd_kernel_error
    ...
    *,
    ...
    if sparsity <= 0.0:
        ...
        if stats is not None and impl == "simd" and chosen != "simd":
            from h3_apple.runtime.sparse import simd_kernel_error
        ...
        if chosen == "simd":
            from h3_apple.runtime.sparse import simd_block_sparse
            ...
            try:
                ...
                raise RuntimeError("Ours sparse attention failed; no dense fallback is enabled") from error
