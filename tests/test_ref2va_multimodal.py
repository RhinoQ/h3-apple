...
@pytest.mark.parametrize("resolution,expected", [("576p", (576, 1024, 576, 1024)), ("768p", (768, 1366, 768, 1376))])
def test_portrait_is_a_native_canvas(resolution, expected):
    request = resolve("a scene", resolution=resolution, aspect_ratio="9:16", seed=42)
    assert (request.width, request.height, request.model_width, request.model_height) == expected


def test_audio_reference_requires_visual_input(monkeypatch):
    monkeypatch.setattr("h3_apple.media.probe_reference", lambda *_: {})
    with pytest.raises(ValueError, match="at least one image or video"):
        resolve("say hello", reference_audio=["voice.wav"])
    request = resolve("say hello", reference_audio=["voice.wav"], reference_videos=["clip.mp4"], seed=42)
    assert request.task == "ref2va" and request.resolution == "576p"
    assert request.reference_audio[0].endswith("voice.wav")
