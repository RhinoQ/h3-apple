...
| Is portrait output available? | This **dev12** build delivers landscape 16:9. A 9:16 candidate is undergoing full-generation validation; it is not available in this build. Portrait source material alone does not select portrait output. |
...
| Can I use a smaller Mac? | Physical testing covers **M5 Max / 128 GiB**. The preview requires M5 and macOS 26.2+. The 64 GiB admission policy comes from constrained-budget text tests, not equivalent physical-machine or reference-generation validation. Check [hardware and memory](../README.md#hardware-and-memory) first. |
