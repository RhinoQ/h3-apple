...
def test_vsa_rejects_missing_gates_before_encoding(tmp_path):
    recipe(tmp_path, 49)
    with pytest.raises(ValueError, match='50 transplanted'):
...
def test_experimental_path_requires_200_actual_sparse_calls(tmp_path, monkeypatch, calls, fallback, passes):
    ...
    monkeypatch.setattr(fl, 'encode_references', lambda *_: (np.ones((3, 5120), np.float32), np.array([1, 0, 1]),
        np.ones((1, 96), np.float32), np.empty((0, 32), np.float32), None, []))
    @dataclass
    class Stats:
